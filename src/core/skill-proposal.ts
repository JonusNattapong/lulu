...
class SkillProposalManager {
  ...
  private createSkillFile(proposal: SkillProposalRecord): void {
    const skillDir = path.join(SKILLS_DIR, proposal.name.replace(/\s+/g, "-").toLowerCase());
    mkdirSync(skillDir, { recursive: true });

    const skillMd = `# ${proposal.name}

> Auto-generated from workflow analysis on ${new Date(proposal.createdAt).toLocaleDateString()}

## Description
${proposal.description}

## Triggers
${proposal.triggers.map(t => `- \`${t}\``).join("\n")}

## Steps
${proposal.steps}

${proposal.examples.length > 0 ? `## Examples\n${proposal.examples.map(e => `- \`${e}\``).join("\n")}` : ""}

---
*Generated by Lulu Personal Agent*
`;
